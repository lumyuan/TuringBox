...
import io.github.lumyuan.turingbox.common.model.BatteryStatus
...
class BatteryUtils {
    ...
    val usbInfo: String
        get() {
            if (RootFile.fileExists("/sys/class/power_supply/usb/uevent")) {
                val batteryInfos = KernelProp.getProp("/sys/class/power_supply/usb/uevent")
                val infos = batteryInfos.split("\n".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
                val stringBuilder = StringBuilder()
                var voltage = 0F
                var electricity = 0F
    ...
    fun getQcLimit(): String {
        ...
        var limit = if (useMainConstant == true) {
            KernelProp.getProp("/sys/class/power_supply/main/constant_charge_current_max")
        } else {
            KernelProp.getProp("/sys/class/power_supply/battery/constant_charge_current_max")
        }
        when {
            limit.length > 3 -> {
    ...
    fun setChargeInputLimit(limit: Int, context: Context, force: Boolean = false): Boolean {
        if (changeLimitRunning && !force) {
            ...
            synchronized(TuringBox.application) {
    ...
    public fun getChargeFull(): Int {
        val value = KernelProp.getProp("/sys/class/power_supply/bms/charge_full")
    ...
    public fun getCpacity(): Int {
        val value = KernelProp.getProp("/sys/class/power_supply/battery/capacity")
    ...
    public fun getKernelCapacity(approximate: Int): Float {
        ...
        if (kernelCapacitySupported == true) {
            try {
                val raw = KernelProp.getProp("/sys/class/power_supply/bms/capacity_raw")
