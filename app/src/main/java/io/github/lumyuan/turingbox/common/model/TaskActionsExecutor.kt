...
class TaskActionsExecutor(
    ...
    private fun updateNotification(text: String) {
        val notice = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            if (!channelCreated) {
                nm.createNotificationChannel(NotificationChannel("tb-task", context.getString(R.string.notice_channel_task), NotificationManager.IMPORTANCE_LOW))
                channelCreated = true
            }
            NotificationCompat.Builder(context, "tb-task")
        } else {
            NotificationCompat.Builder(context)
        }
        nm.notify(920, notice.setSmallIcon(R.drawable.ic_clock).setWhen(System.currentTimeMillis()).setContentTitle(context.getString(R.string.notice_channel_task)).setContentText(text).build())
    }
